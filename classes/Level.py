...
class Level:
    ...
    def loadEntities(self, data):
        ...
        [self.addCoinBox(x, y) for x, y in entities.get("CoinBox", [])]
        [self.addGoomba(x, y) for x, y in entities.get("Goomba", [])]
        [self.addKoopa(x, y) for x, y in entities.get("Koopa", [])]
        [self.addCoin(x, y) for x, y in entities.get("coin", [])]
        [self.addCoinBrick(x, y) for x, y in entities.get("coinBrick", [])]
        [self.addRandomBox(x, y, item) for x, y, item in entities.get("RandomBox", [])]
    ...
    def loadObjects(self, data):
        ...
        self.finishFlag = data["level"]["objects"].get("flag")
        self.pipeWarps = data["level"]["objects"].get("warpPipe", [])
    ...
    def drawLevel(self, camera):
        firstColumn = max(0, 0 - int(camera.pos.x + 1))
        lastColumn = min(self.levelLength, 20 - int(camera.pos.x - 1))

        for y in range(0, 15):
            for x in range(firstColumn, lastColumn):
        ...
        self.drawFinishFlag(camera)

    def drawFinishFlag(self, camera):
        if self.finishFlag is None:
            return

        x, y = self.finishFlag
        screenX = int((x + camera.pos.x) * 32)
        groundY = y * 32
        poleTop = groundY - 9 * 32

        pygame.draw.rect(self.screen, (240, 240, 240), (screenX, poleTop, 5, 9 * 32))
        pygame.draw.circle(self.screen, (240, 240, 240), (screenX + 2, poleTop), 7)
        pygame.draw.polygon(
            self.screen,
            (220, 30, 30),
            [
                (screenX + 5, poleTop + 14),
                (screenX + 74, poleTop + 34),
                (screenX + 5, poleTop + 58),
            ],
        )

    def hasMarioReachedFinishFlag(self, marioRect):
        if self.finishFlag is None:
            return False

        x, y = self.finishFlag
        finishArea = pygame.Rect(x * 32 - 12, (y - 9) * 32, 36, 9 * 32)
        return marioRect.colliderect(finishArea)

    def findPipeWarp(self, marioRect):
        """Returns a destination when Mario presses down while standing on a warp pipe."""
        for warp in self.pipeWarps:
            entryX, entryY = warp["entry"]
            pipeLeft = entryX * 32
            pipeRight = (entryX + 2) * 32
            pipeTop = entryY * 32

            isAbovePipe = abs(marioRect.bottom - pipeTop) <= 4
            isInsidePipe = pipeLeft <= marioRect.centerx <= pipeRight

            if isAbovePipe and isInsidePipe:
                return warp["exit"]
